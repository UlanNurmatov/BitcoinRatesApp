...
class HTTPRequestManager {
    ...
    private func request(method: HTTPMethod, endpoint: String, header: HTTPHeaders?, completion: @escaping SuccessHandler, statusErrorMessage: @escaping FailureHandler, networkError: @escaping NetworkFailureHandler) {
        ...
        Alamofire.request(apiUrl.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed)!, method: method, encoding: URLEncoding.default , headers: header ?? [:]).responseJSON { (response:DataResponse<Any>) in
            ...
            case HTTPStatusCode.ok.statusCode,
                ...
                break
            default:
                networkError(response.error?.localizedDescription
    ...
    internal func get(endpoint: String, header: HTTPHeaders, completion: @escaping SuccessHandler, error: @escaping FailureHandler, networkError: @escaping NetworkFailureHandler) {
        request(method: .get, endpoint: endpoint, header: header, completion: completion, statusErrorMessage: error, networkError: networkError)
    }
